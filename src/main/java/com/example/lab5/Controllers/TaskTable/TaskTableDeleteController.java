...
public class TaskTableDeleteController implements Initializable {
    ...
    public void btnDeleteClick(ActionEvent actionEvent) {
        DataBase.deleteTaskTable(cmbId.getValue());
        errolLog.setText("занятие удалено");
    ...
    void init(){
        var ids = DataBase.getTaskTableId();
        cmbId.setItems(ids);
        if(ids.size()>=1) {
